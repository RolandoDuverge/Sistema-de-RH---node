...
router.get("/createCan", (req, res) => {
  if (req.session.loggedin) {
    conexion.query(
      ...
      (error, results) => {
        conexion.query(
          "SELECT id,descripcion FROM capacitaciones",
          (error, resultsCapa) => {
            conexion.query(
              'SELECT id,nombre FROM puestos WHERE estado = "1"',
              (error, puestoresults) => {
                conexion.query(
                  'SELECT id,descripcion FROM competencias WHERE estado = "1"',
                  (error, resultsCompe) => {
                    conexion.query(
                  ...
                  conexion.query(
                    ...
                    (error, resultscap) => {
                      conexion.query(
                        ...
                        (error, resultscom) => {
                          conexion.query(
                            "SELECT id,empresa FROM experiencia",
                            (error, resultsexp) => {
                              datos = resultsdep;
                              datospu = resultspues;
                              datoscom = resultscom;
    ...
    conexion.query(
      ...
      async (error, results, fields) => {
        if (
          ...
          conexion.query(
            "SELECT rol FROM users WHERE user=? ",
            [user],
            function (err, row) {
              if (row[0].rol == "admin") {
                res.render("login", {
...
router.get("/indexCanUsers", (req, res) => {
  conexion.query(
    'SELECT nombre,nivelRiesgo,nivelSalarioMinimo,nivelSalarioMaximo FROM puestos WHERE estado = "1"',
    (error, puestoresults) => {
      if (req.session.loggedin) {
...
router.get("/indexExpeUsers", (req, res) => {
  conexion.query(
    "SELECT empresa,puesto,fechaPre,fechaPos,salario FROM experiencia",
    (error, results) => {
      if (req.session.loggedin) {
        ...
        conexion.query(
          ...
          (error, resultspues) => {
            conexion.query(
              ...
              (error, resultscap) => {
                conexion.query(
                  "SELECT id,descripcion FROM competencias",
                  (error, resultscom) => {
                    conexion.query(
                      "SELECT id,empresa FROM experiencia",
                      (error, resultsexp) => {
...
router.get("/pushCan/:id", (req, res) => {
  conexion.query(
    "SELECT id,cedula,nombre,departamento,puestoAspira,salarioAspira FROM candidato",
    (error, results) => {
      datos = results;
      const id = req.params.id;
      numero = id - 1;
      ...
      conexion.query(
        ...
        {
          cedula: datos[numero].cedula,
          fechaIngreso: datofecha,
          nombre: datos[numero].nombre,
          departamento: datos[numero].departamento,
          puesto: datos[numero].puestoAspira,
          salarioMensual: datos[numero].salarioAspira,
          estado: datoestado,
        },
        (error, resultsemp) => {
          conexion.query(
